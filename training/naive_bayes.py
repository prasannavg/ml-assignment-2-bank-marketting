"""Gaussian Naive Bayes classifier on the Bank Marketing dataset."""
...
def run_training():
    ...
    train_x_dense = np.asarray(train_x, dtype=np.float64)
    test_x_dense = np.asarray(test_x, dtype=np.float64)

    clf = GaussianNB()
    clf.fit(train_x_dense, train_y)

    scores = score_predictions(clf, test_x_dense, test_y)
